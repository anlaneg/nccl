...
static ncclResult_t canConnect(int* ret/*出参，是否容许连接*/, struct ncclComm* comm, struct ncclTopoGraph* graph, struct ncclPeerInfo* info1,
...
static ncclResult_t sendSetup(struct ncclComm* comm, struct ncclTopoGraph* graph, struct ncclPeerInfo* myInfo/*本端信息*/,
                              struct ncclPeerInfo* peerInfo/*对端信息*/, struct ncclConnect* connectInfo,
      ...
      if (sub->transmitted < sub->posted && sub->transmitted < sub->done + NCCL_STEPS) {
        ...
        if (connFifo[buffSlot].size != -1 && (*recvTail > tail || p == NCCL_PROTO_LL)) {
          ...
          if (ready) {
            ...
            /*发送buffer*/
            NCCLCHECK(proxyState->ncclNet->isend(resources->netSendComm, buff/*要发送的内容*/, size, resources->tpRank,
