...
template <typename T>
ncclResult_t ncclCallocDebug(T** ptr, size_t nelem/*元素数目*/, const char* file, int line, const char* callerFunc,
                             bool logHostAlloc) {
  if (nelem > 0) {
...
#define ncclRealloc(ptr/*入出参*/, oldNelem/*旧数目*/, nelem) \
...
inline ncclResult_t ncclIbMallocDebug(void** ptr/*出参，申请到的内容*/, size_t size, const char* file, int line, const char* callerFunc) {
  if (size > 0) {
    void* p = NULL;
    size_t page_size = ncclOsGetPageSize();/*取页大小*/
#if defined(NCCL_OS_LINUX)
    size_t size_aligned = ROUNDUP(size, page_size);/*按页对齐*/
